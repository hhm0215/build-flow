...
- **상태**: IN_PROGRESS (현장 생성·거래처 등록·견적 확정·현장 수정·DRAFT 견적 수정/삭제·보증보험 OCR 실패 수동 보정/수정·세금계산서 입금 확인 계약/오류 처리·Kafka 손익 신뢰성 Phase 1~3·확정 상태 서버 변경 보호·매입 금액 검증 및 수정/삭제 UI 완료. 다음은 세금계산서 금액 검증 및 수정/삭제 UI)
