...
## 능동 발의 (Initiative) — 실험 운영 2026-07-01 시작

> **실험 중** — 다음 세션 시작 시 회고 대상. 결과에 따라 확정/폐기/수정. 상태는 ADR-014 참조.
...
발의할 때마다 `.claude/PROGRESS.md`의 "능동 발의 로그 (실험 기간)" 섹션에 한 줄 축적:

```
- [YYYY-MM-DD HH:MM] {발의 종류} — 판단: … / 근거: … / 대안: … / 응답: 승인|거부|수정
```

**회고 트리거**(2026-07-03 조정): 발의 5건 축적 또는 실사이클 3회 경과 중 먼저 도달하는 시점에 이 로그를 근거로 실험 회고(승인률·거부 사유·정합성 이탈) 후 확정/폐기/수정 결정. (이전엔 "다음 세션 시작"이었으나 도입 세션에 사이클이 안 남으면 발의 0건으로 공회전 → 데이터 축적 기준으로 전환)
...
- **IMPORTANT**: 발의 없이 큰 결정을 지시대로만 실행하면 이 실험 실패. 4가지 트리거 조건 도래 시 반드시 발의 먼저.
- **IMPORTANT**: 발의를 던져놓고 실행 게이트를 사용자 승인 없이 통과하면 실험 실패. 발의 ≠ 자율 실행 확대.
