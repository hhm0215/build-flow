...
## 7. 장기 제품 구조와 현재 아키텍처의 경계

현재 서비스 경계는 건설 현장 업무를 위한 구현이며 범용 정보관리 코어가 구현된 상태는 아니다. 장기 확장은 `docs/PRODUCT_VISION.md`의 단계에 따라 다음 논리 계층으로 검증한다.
...
[가져오기 어댑터]
        ...
        ▼
[범용 정보 코어 후보]
  원본 · 출처 · 버전 · 관계 · 확장 메타데이터 · 감사
        │
        ├──────────────┐
        ▼              ▼
[건설 도메인 팩]   [향후 다른 도메인 팩]
 현장·견적·손익      유형·필드·업무 규칙
        │              │
        └──────┬───────┘
               ▼
       [근거 기반 Assistant]
...
첫 단계에서는 별도 마이크로서비스를 선제적으로 추가하지 않는다. 실제 USB 파일럿으로 원본 저장·가져오기·관계 검토 경계를 검증한 뒤 기존 서비스 내부 모듈로 둘지 독립 서비스로 분리할지 결정한다. 객체 저장소, 벡터 DB, 멀티테넌시 역시 데이터 규모와 두 번째 도메인 요구가 확인될 때 ADR로 확정한다. 건설과 두 번째 실제 도메인에서 같은 의미로 재사용된 공통 기반만 범용 코어로 승격한다.
...
|------|------|------|
| v1.0 | 2026-04-04 | 초안 |
| v1.1 | 2026-09-23 | 장기 정보관리 플랫폼 논리 계층과 현재 구현 경계 추가 |
