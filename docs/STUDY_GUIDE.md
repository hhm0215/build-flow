...
> 이 문서의 회원가입·이메일 로그인·VIEWER 예시는 초기 구현 설명으로 현재 인증 계약과 다릅니다. 현재는 단일 관리자 `loginId/password`, 로컬 대화형 초기화, 공개 회원가입 없음입니다. 최신 계약은 `docs/API_SPEC.md`의 auth-service 절을 우선하세요.
...
- 공급가액·세액은 각각 0 이상·정수 13자리/소수 2자리 이하이고 합계도 `DECIMAL(15,2)` 범위를 넘을 수 없다. API 경계와 Entity에서 이 규칙을 이중 보호해 직접 호출과 내부 생성 모두 같은 불변식을 지킨다.
- 입금 확인은 매출 세금계산서에만 허용한다. 입금 확인 후에는 금액·거래처 등 수정과 삭제를 할 수 없으며 서버가 `409 Conflict`로 거부한다. 정정·취소는 감사 가능한 별도 workflow를 설계하기 전까지 제공하지 않는다.
