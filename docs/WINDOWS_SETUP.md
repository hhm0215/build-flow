...
fresh DB에는 계정 seed가 없고 공개 회원가입 API도 없습니다. 전체 스택이 올라온 뒤 로컬 대화형 스크립트로 최초 관리자 계정을 한 번 생성합니다. 아이디는 영문·숫자·점·밑줄·하이픈 3~50자, 비밀번호는 9자 이상입니다. 비밀번호는 마스킹 입력되며 명령줄 인수나 환경변수로 전달되지 않습니다.

```powershell
.\scripts\create-admin.ps1
```

그다음 http://localhost:3000 에서 생성한 관리자 아이디와 비밀번호로 로그인합니다. 가족이 같은 관리자 계정을 공유합니다.

실서비스 로그인·현장 API 연결을 한 번에 확인하려면 다음 명령을 실행합니다. 비밀번호는 마스킹 입력되고 검증용 현장 1건은 생성 직후 삭제됩니다.

```powershell
.\scripts\verify-admin.ps1
```

기존 개발 DB에 옛 `users` 계정이 있어도 자동 삭제하지 않습니다. 스크립트는 새 `admin_accounts` 테이블에 계정이 이미 있으면 거부합니다. 이전 데이터를 보존해야 한다면 DB를 백업하고 전환 절차를 먼저 확인하세요.
